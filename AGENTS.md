...
.agents/skills/                           # Repo-scope agent skills (canonical; .claude/skills is a symlink to it)
scripts/
  wire-app-agents.sh                      # Post-scaffold: wires <AppName>'s agent files into Claude Code
AGENTS.md                                 # This file (canonical; CLAUDE.md is a symlink to it)
<AppName>/                                # Rayfin-scaffolded data app (created during the workflow)
  AGENTS.md                               # App-scope instructions (scaffold-owned; CLAUDE.md symlinked by wire script)
  .agents/skills/                         # App-scope skills (scaffold-owned; .claude/skills symlinked by wire script)
```
...
- After every `ai-files install`, run `scripts/wire-app-agents.sh <AppName>` from the repo root — it wires the scaffold's agent files into Claude Code discovery (see "Two instruction scopes" below)
...
### Two instruction scopes (agentcanon routing)

This repo has two agent-instruction sets. They are **scopes, not duplicates** — per the AGENTS.md standard, a nested file governs its own subtree:

- **Repo scope** (this file + `.agents/skills/`): model conventions, workflow, and the precedence rules below. Not regenerated by any tool — durable overrides of scaffold behavior belong **here**, never in the nested files.
- **App scope** (`<AppName>/AGENTS.md` + `<AppName>/.agents/skills/`): Rayfin's instructions and skills. Scaffold-owned — `npx rayfin ai-files install` rewrites them, so never edit them expecting the change to stick.

Claude Code reads only `CLAUDE.md`/`.claude/skills` paths, so each scope carries symlinks to its canonical files: at the root they're committed; inside `<AppName>/` they're created by `scripts/wire-app-agents.sh <AppName>` (`CLAUDE.md → AGENTS.md`, `.claude/skills → ../.agents/skills`). Claude Code loads nested files lazily — app-scope instructions and skills activate only when working on files under `<AppName>/`, which is exactly the scope they apply to. Keep real content in the `.agents`/`AGENTS.md` locations; never create real files at a `.claude` path.
